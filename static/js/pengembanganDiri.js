...
class DevelopmentManager {
    ...
    items.forEach((item) => {
      ...
      li.innerHTML = `
        <div>
          ...
          ${item.description ? 
            `<p class="item-desc">${item.description.length > 150 ? 
  ...
  renderChart() {
    ...
    const totalItems = Object.keys(CATEGORIES).reduce(
      (acc, key) => acc + (this.userData[key]?.length || 0),
      0
    );
    ...
    Object.entries(CATEGORIES).forEach(([key, category]) => {
      ...
      const percentage = target > 0 
        ...
        : totalItems > 0 ? (count / totalItems) * 100 : 0;
  ...
  exportToCSV() {
    ...
    Object.keys(CATEGORIES).forEach((category) => {
      ...
      items.forEach((item) => {
        csvContent.push(
          `"${CATEGORIES[category].label}","${item.title}",` +
  ...
  try {
    ...
    const filteredItems = this.userData[category].filter(
      (item) => item.id !== itemId
    );
