...
class DevelopmentManager {
    ...
    async authStateHandler() {
		  auth.onAuthStateChanged(async (user) => {
		    ...
		    this.setupRealtimeListener(); // Hapus pemanggilan loadUserData
		  });
		}
    handleNextStep() {
    ...
    async handleFormSubmit(e) {
		    ...
		    try {
		        ...
		        // Memperbarui UI setelah data disimpan
    ...
    renderDataSections() {
        ...
        Object.entries(CATEGORIES).forEach(([key, category]) => {
            const section = this.dom.sectionTemplate.content.cloneNode(!0);
    ...
    handleGlobalSearch(e) {
        ...
        // Di dalam method handleGlobalSearch()
				Object.keys(CATEGORIES).forEach(category => {
				  const items = this.userData[category] || [];
				  ...
				  // Sembunyikan section jika tidak ada hasil
    ...
    setupLiveValidation() {
        document.querySelectorAll("[required]").forEach((input) => {
            input.addEventListener("input", () => {
                input.style.borderColor = input.checkValidity() ? "var(--border)" : "var(--error)";
            });
        });
    }
    ...
    setDefaultDate() {
        ...
        this.dom.tanggalInput.value = localDate;
    }
    ...
    async handleUpdateItem(e) {
        ...
        try {
            ...
            const updates = { title: document.getElementById("edit-title").value, date: Timestamp.fromDate(editDate), description: document.getElementById("edit-desc").value, updatedAt: Timestamp.now() };
    ...
    async setupRealtimeListener() {
		  ...
		  onSnapshot(this.userRef, (doc) => {
		    if (doc.exists()) {
		      ...
		      // Periksa perubahan untuk semua kategori dan target
		      ...
		      // Handle pencarian global jika ada filter
		      if (this.dom.globalSearch.value) {
		        this.handleGlobalSearch({ target: this.dom.globalSearch });
		      }
		    }
		  });
		}
